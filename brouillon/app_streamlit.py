...
from connection_llm import extract_skills
...
# Initialiser les nouvelles colonnes dans le DataFrame 
df["Hard Skills"] = "" 
df["Soft Skills"] = "" 
df["Technologies"] = ""

# Extraire les compétences pour chaque offre d'emploi et mettre à jour le DataFrame 
for index, row in df.iterrows(): 
    job_description = row["description"] 
    hard_skills, soft_skills, technologies = extract_skills(job_description) 
    df.at[index, "Hard Skills"] = ", ".join(hard_skills) 
    df.at[index, "Soft Skills"] = ", ".join(soft_skills) 
    df.at[index, "Technologies"] = ", ".join(technologies)
...
def pie_chart(final_df, titre_poste, lieuTravail):
...
pie_chart(final_df, titre_poste, lieuTravail)
...
#ax.tick_params(axis='x', rotation=45, ha= 'right')  # Rotation des labels des abscisses pour éviter le chevauchement
